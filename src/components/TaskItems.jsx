...
export const TaskItem = ({ task, onDelete, onToggle }) => (
  ...
  >
    <div
      className={`flex-1 ${task.completed ? "line-through text-white" : "text-white font-medium"
        }`}
    >
      <label className="flex items-center gap-3 cursor-pointer">
        ...
        />
        <span
          className={`w-5 h-5 flex items-center justify-center border-1 rounded-md transition-all
        ${task.completed
              ? "bg-[#FF8A59] border-[#FF8A59]"
        ...
        <span
          className={`whitespace-normal break-words w-full text-sm sm:text-base ${task.text.length > 5 ? "max-w-[250px]" : ""
            }`}
        >
          {task.text}
        </span>
